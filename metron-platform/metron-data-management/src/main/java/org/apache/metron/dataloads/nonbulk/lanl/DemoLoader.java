 ...
 */
package org.apache.metron.dataloads.nonbulk.lanl;
...
public class DemoLoader {
  ...
  private static KafkaProducer createProducer(List<String> brokers, Map<String, Object> config) {
  ...
  public static void main(String... argv) throws Exception {
    ...
    if(LoadOptions.HOSTNAME_FILTER.has(cli)) {
      try(BufferedReader br = new BufferedReader(new FileReader(new File(LoadOptions.HOSTNAME_FILTER.get(cli))))) {
  ...
  public static void step( int start
    ...
    for(DemoConfig.DataSource ds : config.getSources()) {
      ...
      if(ds.getInputFile().endsWith(".gz")) {
        reader = new BufferedReader(new InputStreamReader(new GZIPInputStream(new FileInputStream(ds.getInputFile()))));
      }
      else {
        reader = new BufferedReader(new FileReader(ds.getInputFile()));
      }
